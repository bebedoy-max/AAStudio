...
function parseGptQuality(input: string): {
  quality: string;
  ...
  const modelNode = {
    ...
    data: {
      ...
      kind: {
        ...
        inputs: [[{ id: "prompt", title: "prompt", validTypes: ["text"], required: true }, null]],
        parameters: [
          [
            {
              id: "prompt",
...
export async function generateWeavyImage(opts: WeavyImgOpts): Promise<string> {
  ...
  if (isSeedream) {
    const { buildSeedreamEditRecipe, resolveSeedreamImageSize } =
      await import("./weavy-storyboard");
    ...
    built = buildGptImage2Recipe(
      opts.prompt,
      opts.quality || "high@1024x1024",
      opts.ratio || "1:1",
    );
  }
  ...
  while (true) {
    ...
    try {
      ...
      log(
        `↻ token ditolak (${msg.slice(0, 80)}${bal !== null ? ` · saldo ${bal} cr` : ""}), rotate ke token berikutnya…`,
      );
    }
  }
  if (highestSkippedBalance > 0) {
    throw new Error(
      `Tidak ada token Weavy dengan saldo minimal ${requiredCredits} cr (saldo tertinggi ${highestSkippedBalance} cr).`,
    );
  }
