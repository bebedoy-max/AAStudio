    ...
    data: {
      ...
      kind: {
        ...
        inputs: [
          ...
          [
            { id: "image", title: "image", validTypes: ["image"], required: true },
            { nodeId: n1, outputId: "file" },
          ],
          [
            { id: "image_2", title: "image_2", validTypes: ["image"], required: false },
          ...
          [
            {
              ...
              defaultValue: { type: "list", value: imageRefs },
            },
            { type: "value", data: { type: "list", value: imageRefs } },
          ],
          [
            {
              id: "prompt",
          ...
          [
            {
              ...
              title: "aspect_ratio",
              constraint: { type: "enum" },
              defaultValue: { type: "string", value: ratio },
            },
...
function buildGptImage2EditBulkRecipe(
  prompt: string,
  quality: string,
  ratio: string,
  charUrl: string,
          ...
          [
            {
              ...
              title: "image_size",
              constraint: { type: "any" },
              defaultValue: { type: parsed.size ? "object" : "string", value: imageSize },
            },
            { type: "value", data: { type: parsed.size ? "object" : "string", value: imageSize } },
          ],
          [
            {
              id: "num_images",
...
function buildSeedreamEditBulkRecipe(
  prompt: string,
  modelKey: string,
  ratio: string,
  charUrl: string,
    ...
    data: {
      ...
      kind: {
        ...
        inputs: [
          [{ id: "prompt", title: "prompt", validTypes: ["text"], required: true }, null],
          [
            { id: "image_1", title: "image_1", validTypes: ["image"], required: false },
            { nodeId: n1, outputId: "file", file: slot1 },
          ],
          [
            { id: "image_2", title: "Image 2", validTypes: ["image"], required: false },
            { nodeId: n2, outputId: "file", file: slot2 },
          ],
        ],
        parameters: [
          [
...
export async function generateWeavyBulkOne(opts: WeavyBulkOpts): Promise<string> {
  ...
  while (true) {
    ...
    tried.add(active.id);
    try {
      const charUp = await uploadWeavyAssetWithRetry(
        opts.charFile,
        `char_${Date.now()}.jpg`,
        active.accessToken,
      );
      const charUrl = resolveWeavyAssetUrl(charUp, "image");
      const outUp = await uploadWeavyAssetWithRetry(
        opts.outfitFile,
        `outfit_${Date.now()}.jpg`,
        active.accessToken,
      );
      const outUrl = resolveWeavyAssetUrl(outUp, "image");

      if (opts.modelKey.startsWith("seedream-")) {
        const sb = buildSeedreamEditBulkRecipe(
          opts.prompt,
          opts.modelKey,
          opts.ratio || "1:1",
          charUrl,
        ...
        await saveWeavyRecipe(
          rid,
          { nodes: sb.nodes, edges: sb.edges, v3: sv3 },
          active.accessToken,
        );
        await approveWeavyModel(sb.model, active.accessToken);
        const { batchId: bid } = await executeWeavyBatch(
          rid,
          sb.nodes,
          sb.edges,
          active.accessToken,
