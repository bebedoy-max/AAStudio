  ...
  head: () => ({
    meta: [
      { title: "AI Upscaler & Enhancer — AA Creative Studio" },
      {
        name: "description",
        content:
          "Upscale / enhance gambar (satuan atau bulk s/d 50 gambar) menggunakan Topaz atau Magnific.",
      },
    ],
  }),
...
function UpscalerPage() {
  ...
  useEffect(() => {
    upscalerSession.rows = rows;
  }, [rows]);
  useEffect(() => {
    upscalerSession.logs = logs;
  }, [logs]);
  useEffect(() => {
    upscalerSession.progress = progress;
  }, [progress]);
  useEffect(() => {
    upscalerSession.running = running;
  }, [running]);
  useEffect(() => {
    upscalerSession.provider = provider;
  }, [provider]);
  useEffect(() => {
    upscalerSession.mode = mode;
  }, [mode]);
  ...
  const gallery = useCloudGallery<{ provider?: string; mode?: string; sourceName?: string }>(
    "upscaler",
    "image",
  );
  ...
  const pickImages = () =>
    void picker
      .pick({ accept: "image/*", multiple: true, source: "upscaler", title: "Pilih gambar" })
      .then((fs) => {
  ...
  async function handleRun() {
    ...
    setRows((prev) =>
      prev.map((r) => ({ ...r, status: "queued", url: undefined, error: undefined })),
    );
    ...
    try {
      await runUpscale(jobs, {
        ...
        topaz: {
          model: tzModel,
          upscale_factor: tzFactor,
          output_format: tzFormat,
          crop_to_fill: tzCrop,
        },
        magnific: {
        ...
        onStatus: ({ index, status, url, error }) => {
          setRows((prev) =>
            prev.map((r, i) =>
              i === index ? { ...r, status, url: url ?? r.url, error: error ?? r.error } : r,
            ),
          );
          if (status === "done" && url) {
  ...
  const filteredResults = results.filter(
    (r) => !search || r.sourceName.toLowerCase().includes(search.toLowerCase()),
  );
    ...
    try {
      ...
      await downloadFilesAsZip(
        filteredResults.map((r, i) => {
          ...
          return {
            url: r.url,
            filename: `upscale-${String(i + 1).padStart(2, "0")}-${r.id}.${ext.toLowerCase()}`,
          };
        }),
            ...
            <Card title="Konfigurasi" sub="Pilih model AI & parameter">
              <div className="space-y-4">
                  ...
                  <>
                    ...
                    <label className="flex items-center gap-2 text-sm text-foreground/90">
                      <input
                        type="checkbox"
                        checked={tzCrop}
                        onChange={(e) => setTzCrop(e.target.checked)}
                      />
                      Crop to fill
                    </label>
                  </>
                  ...
                  <>
                    <Field label="Scale factor">
                      <Select
                        value={magScale}
                        onChange={(e) =>
                          setMagScale(e.target.value as MagnificParams["scale_factor"])
                        }
                        options={[
                          { value: "2x", label: "2x" },
                          { value: "4x", label: "4x" },
                    ...
                    <Field label="Optimized for">
                      <Select
                        value={magOpt}
                        onChange={(e) =>
                          setMagOpt(e.target.value as MagnificParams["optimized_for"])
                        }
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <Field label={`Creativity (${magCreativity})`}>
                        <Input
                          ...
                          max={10}
                          value={magCreativity}
                          onChange={(e) => setMagCreativity(Number(e.target.value))}
                        />
                      </Field>
                      <Field label={`HDR (${magHdr})`}>
                        <Input
                          ...
                          max={10}
                          value={magHdr}
                          onChange={(e) => setMagHdr(Number(e.target.value))}
                        />
                      </Field>
                      <Field label={`Resemblance (${magResemblance})`}>
                        <Input
                          ...
                          min={0}
                          max={100}
                          value={magResemblance}
                          onChange={(e) => setMagResemblance(Number(e.target.value))}
                        />
                      </Field>
                      <Field label={`Fractality (${magFractality})`}>
                        <Input
                          ...
                          max={10}
                          value={magFractality}
                          onChange={(e) => setMagFractality(Number(e.target.value))}
                        />
                      </Field>
                    </div>
                    <Field label="Prompt (opsional)">
                      <Textarea
                        rows={2}
                        value={magPrompt}
                        onChange={(e) => setMagPrompt(e.target.value)}
                  ...
                  <>
                    <Field label="Upscaler">
                      <Select
                        value={leoUpscaler}
                        onChange={(e) =>
                          setLeoUpscaler(e.target.value as LeonardoAuroraParams["upscaler"])
                        }
                        options={[
                    ...
                    {leoUpscaler === "pro" && (
                      <Field label="Type">
                        <Select
                          value={leoProType}
                          onChange={(e) =>
                            setLeoProType(e.target.value as LeonardoAuroraParams["pro_type"])
                          }
                          options={[
                    ...
                    <Field label={`Upscale Multiplier (${leoFactor}x)`}>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const snapped = (
                            raw === 7 ? 8 : raw
                          ) as LeonardoAuroraParams["upscale_factor"];
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      Multiplier Leonardo: 2, 3, 4, 5, 6, atau 8x. Jika hasil melewati limit Aurora
                      ±105MP, sistem otomatis menurunkan multiplier supaya job tidak gagal.
                    </p>
                  </>
                )}
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <GhostButton onClick={clearAll} disabled={running || rows.length === 0}>
                    Bersihkan
                  </GhostButton>
                </div>
              </div>
          ...
          <div className="rounded-xl border border-border/60 bg-black/40 p-2 max-h-64 overflow-y-auto overflow-x-hidden text-[11px] font-mono min-w-0">
            {logs.length === 0 ? (
              <div className="text-muted-foreground px-1 py-2">
                Belum ada log. Jalankan proses untuk melihat aktivitas.
              </div>
            ) : (
              logs.map((l, i) => (
                <div
              ...
              {filteredResults.map((r) => (
                ...
                >
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="block relative bg-black/40"
                  >
                  ...
                  <div className="p-2 text-[11px] text-muted-foreground flex items-center justify-between gap-1">
                    <span className="truncate flex-1" title={r.sourceName}>
                      {r.sourceName}
                    </span>
                    <button
                      type="button"
