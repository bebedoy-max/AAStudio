  ...
  head: () => ({
    meta: [
      { title: "Log Aktivitas — Admin AA Creative Studio" },
      {
        name: "description",
        content:
          "Lihat semua aktivitas pengguna dengan filter kategori, aksi, user, dan rentang tanggal.",
      },
    ],
  }),
  component: Page,
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`text-[10px] font-mono uppercase tracking-widest px-2 py-0.5 rounded-full border ${cls}`}
    >
      {status}
    </span>
  );
              ...
              {exportOpen && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 z-40 w-56 neumorph p-1">
                    <button
                      onClick={exportCSV}
                      className="w-full text-left px-3 py-2 rounded-lg text-sm hover:bg-sidebar-accent/60"
                    >
                      Excel / CSV (.csv)
                    </button>
                    <button
              ...
              >
                <div className="px-5 py-4 border-b border-border/60">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {new Date(mobileRow.created_at).toLocaleString("id-ID", {
                      dateStyle: "long",
                      timeStyle: "medium",
                    })}
                  </div>
