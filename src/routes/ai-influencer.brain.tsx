      ...
      while (true) {
        ...
        for (const p of parts) {
          ...
          try {
            ...
            if (
              evt.status === "error" &&
              (evt.step === "extract_persona" ||
                evt.step === "extract_memory" ||
                evt.step === "auth" ||
                evt.step === "error")
            ) {
            ...
            {logs.map((l, i) => (
              <li key={i} className="flex items-start gap-2 min-w-0">
                {(() => {
                  const later = logs
                    .slice(i + 1)
                    .find((next) => next.step === l.step && next.detail === l.detail);
                      ...
                      (busy && logs.length > 0 ? (
                        <span className="inline-flex items-center gap-1.5">
                          ...
                          <span className="text-primary/90">
                            [{logs[logs.length - 1].step}] {logs[logs.length - 1].label} ·{" "}
                            {progress}%
                          </span>
                        </span>
                      ) : (
            ...
            {MEMORY_KEYS.map((k) => (
              <li key={k} className="flex items-start gap-2">
                ...
                <div>
                  ...
                  {memory?.[k] && (
                    <div className="text-[11px] text-muted-foreground">{memory[k]}</div>
                  )}
                </div>
              </li>
            ))}
        ...
        <Card title="AI Learning" sub="Sumber data yang dianalisa.">
          {learning ? (
            <div className="space-y-2 text-xs">
              <div>
                Sumber dianalisa:{" "}
                <b>{String((learning as { scraped_count?: number }).scraped_count ?? 0)}</b>
              </div>
              <ul className="space-y-1 max-h-40 overflow-y-auto">
                {Array.isArray(
                  (learning as { sources?: { platform: string; url: string }[] }).sources,
                ) &&
                  (learning as { sources: { platform: string; url: string }[] }).sources.map(
                    (s, i) => (
            ...
            <div className="flex flex-col items-center justify-center py-10 text-center gap-3">
              ...
              <div className="text-xs text-muted-foreground max-w-xs">
                Klik <b>Analisa & Build Brain</b> untuk mulai — pipeline akan scrape sosmed & isi
                memory otomatis.
              </div>
            </div>
          )}
