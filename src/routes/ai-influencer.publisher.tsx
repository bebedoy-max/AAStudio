...
import {
  ...
  connectPublisherAccount,
  disconnectPublisherAccount,
  loadStrategy,
  saveQueueBatch,
...
function PublisherPage() {
  ...
  useEffect(() => {
    reloadAccounts(); /* eslint-disable-next-line */
  }, []);
  ...
  const submitConnect = async () => {
    ...
    if (!connectForm.handle.trim()) {
      toast.error("Handle wajib diisi.");
      return;
    }
    try {
      await _connect({
  ...
  const generateAiSchedule = async () => {
    if (!activeId) {
      toast.error("Pilih karakter dulu.");
      return;
    }
    if (accounts.length === 0) {
      toast.error("Connect minimal 1 platform dulu.");
      return;
    }
    setGenBusy(true);
    try {
  ...
  const saveApproved = async () => {
    ...
    if (approved.length === 0) {
      toast.error("Tidak ada draft yang di-approve.");
      return;
    }
    setSaveBusy(true);
    try {
                    ...
                    {connected.map((a) => (
                      <li key={a.id} className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2 min-w-0">
                          <Chip tone={a.status === "connected" ? "success" : "warn"}>
                            {a.status}
                          </Chip>
        ...
        right={
          <div className="flex gap-2">
            <PrimaryButton
              ...
              disabled={!activeId || genBusy || accounts.length === 0}
            >
              {genBusy ? (
          ...
          <>
            <div className="mb-3 flex gap-2 flex-wrap">
              <GhostButton onClick={approveAll}>
                <Check className="h-4 w-4" /> Approve semua
              </GhostButton>
                ...
                >
                  ...
                  <div className="mt-2 flex gap-1.5 justify-end">
                    <button
                      ...
                      style={
                        d.action === "approve" ? { background: "var(--gradient-neon)" } : undefined
                      }
                    >
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Field label="Platform">
            <Select
              options={[
                { value: "", label: "— pilih —" },
                ...PUBLISH_PLATFORMS.map((p) => ({ value: p.key, label: p.label })),
              ]}
            />
          </Field>
          <Field label="Caption">
            ...
            <div className="p-6">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <DialogDescription className="text-xs text-muted-foreground mt-1 text-left">
                      Simpan handle + access token / webhook. OAuth resmi butuh review platform —
                      sementara pakai integrasi via Zapier / Make.
                    </DialogDescription>
                  </div>
                </div>
              </DialogHeader>
              <div className="space-y-3">
                ...
                <Field
                  ...
                  hint="URL dari Zapier / Make yang akan menerima payload post."
                >
                  <Input
                ...
                <Field
                  ...
                  hint="Untuk integrasi via API resmi (bila tersedia)."
                >
                  <Input
